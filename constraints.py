...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for x in qi.curDomain():
            for y in qj.curDomain():
                if x != y and abs(x - y)!= abs(i - j):
                    satisfyingAssignments.append([x,y])
                    satisfyingAssignments.append([y,x])
    ...
    def check(self):
        ...
        for v in self.scope():
            if v.isAssigned() and v.getValue() in self._required:
                ...
                return True
        return self._lb <= satisfied <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def testPartial(l):
            ...
            for (var, val) in l:
                if val in self._required:
                    satisfied += 1
            return satisfied <= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], testComplete, testPartial)
        return x

class checkAllFlights(Constraint):
    ...
    def check(self):
        ...
        for v in self.scope():
            if v.isAssigned() and v.getValue != "NOFLIGHT":
                val = v.getValue()
        ...
        return len(assignments) == len(self._value)
    ...
    def hasSupport(self, var, val):
            ...
            for (var, val) in l:
                if val != "NOFLIGHT":
                    if val in flights:
                        ...
                        flights.add(val)
            return len(flights) == len(self._value)

        def partialCoverage(l):
            flights = set()
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], completeCoverage, partialCoverage)
        return x